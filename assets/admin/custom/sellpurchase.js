$(document).ready(function() {
    ...
    $('form').parsley();
    var gst=$('.trgst').html();
    var igst='<input type="number" name="igst[]" class="form-control sIgst "  step="any"  placeholder="IGST" readonly /> <input type="hidden" name="igstp[]" class="form-control sIgstp w-50"  step="any"  placeholder="CGST" readonly />';
    ...
    var last_tr=$('#lsttr').html();
      ...
      $('body').on('change','.sProduct_id', function(){
				...
				ProductPrice = $(this).parents('tr').find('.stotal').val() * 1;
			
				$.ajax({
                    type: "POST",
                       url: "../SellPurchase/get_Gst/"+Product_Id+"",
					success: function(result)
					{
            result = result.replace(/\s+/, "");
						data = result.split("###");
						var cgst_percentage = data[0];
						var sgst_percentage = data[1];
            var igst_percentage = data[2];    
           
            $(parent).find('.sSgstp').val(cgst_percentage);
            $(parent).find('.sCgstp').val(sgst_percentage);
            $(parent).find('.sIgstp').val(igst_percentage);
...
function callByprice($obj) {
  var quality= $obj.find('.sQuality').val();
  var sgst_percentage= $obj.find('.sSgstp').val();
  var cgst_percentage= $obj.find('.sCgstp').val();
  var igst_percentage= $obj.find('.sIgstp').val();
  var g_type=$('#gst_type').val();
  var Touch=$('.Touch').val();
  if(!g_type){
